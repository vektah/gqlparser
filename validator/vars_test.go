...
func TestValidateVars(t *testing.T) {
	...
	t.Run("input object", func(t *testing.T) {
		t.Run("non object", func(t *testing.T) {
			...
			q := gqlparser.MustLoadQuery(
				schema,
				`query foo($var: InputType!) { structArg(i: $var) }`,
			)
			_, gerr := validator.VariableValues(
				schema,
				q.Operations.ForName(""),
		...
		t.Run("defaults", func(t *testing.T) {
			...
			q := gqlparser.MustLoadQuery(
				schema,
				`query foo($var: InputType! = {name: "foo"}) { structArg(i: $var) }`,
			)
			vars, gerr := validator.VariableValues(schema, q.Operations.ForName(""), nil)
			require.NoError(t, gerr)
	...
	t.Run("array", func(t *testing.T) {
		t.Run("non-null object value should be coerced to an array", func(t *testing.T) {
			...
			q := gqlparser.MustLoadQuery(
				schema,
				`query foo($var: [InputType!]) { arrayArg(i: $var) }`,
			)
			vars, gerr := validator.VariableValues(
				schema,
				q.Operations.ForName(""),
		...
		t.Run("non-null int deep value should be coerced to an array", func(t *testing.T) {
			...
			q := gqlparser.MustLoadQuery(
				schema,
				`query foo($var: [CustomType]) { typeArrayArg(i: $var) }`,
			)
			vars, gerr := validator.VariableValues(
				schema,
				q.Operations.ForName(""),
		...
		t.Run("defaults", func(t *testing.T) {
			...
			q := gqlparser.MustLoadQuery(
				schema,
				`query foo($var: [InputType!] = [{name: "foo"}]) { arrayArg(i: $var) }`,
			)
			vars, gerr := validator.VariableValues(schema, q.Operations.ForName(""), nil)
			require.NoError(t, gerr)
		...
		t.Run("invalid variable paths", func(t *testing.T) {
			...
			q := gqlparser.MustLoadQuery(
				schema,
				`query foo($var1: InputType!, $var2: InputType!) { a:structArg(i: $var1) b:structArg(i: $var2) }`,
			)
			_, gerr := validator.VariableValues(
				schema,
				q.Operations.ForName(""),
	...
	t.Run("String Array", func(t *testing.T) {
		t.Run("Array with null", func(t *testing.T) {
			...
			q := gqlparser.MustLoadQuery(
				schema,
				`query foo($var: [String]) { stringArrayArg(i: $var) }`,
			)
			a := "1"
			b := "2"
	...
	t.Run("Boolean Array", func(t *testing.T) {
		t.Run("Array with null", func(t *testing.T) {
			...
			q := gqlparser.MustLoadQuery(
				schema,
				`query foo($var: [Boolean]) { boolArrayArg(i: $var) }`,
			)
			a := true
			b := false
...
func TestValidateVarsWithRules(t *testing.T) {
	...
	t.Run("undefined variable", func(t *testing.T) {
		t.Run("without default", func(t *testing.T) {
			q := gqlparser.MustLoadQueryWithRules(
				schema,
				`query($id: Int!) { intArg(i: $id) }`,
				nil,
			)
			_, gerr := validator.VariableValues(schema, q.Operations.ForName(""), nil)
			require.EqualError(t, gerr, "input: variable.id must be defined")
	...
	t.Run("array", func(t *testing.T) {
		...
		t.Run("non-null int value should be coerced to an array", func(t *testing.T) {
			q := gqlparser.MustLoadQueryWithRules(
				schema,
				`query foo($var: [Int!]) { intArrayArg(i: $var) }`,
				nil,
			)
			vars, gerr := validator.VariableValues(
				schema,
	...
	t.Run("Scalars", func(t *testing.T) {
		t.Run("String -> String", func(t *testing.T) {
			q := gqlparser.MustLoadQueryWithRules(
				schema,
				`query foo($var: String!) { stringArg(i: $var) }`,
				nil,
			)
			vars, gerr := validator.VariableValues(
				schema,
		...
		t.Run("Undefined -> Int", func(t *testing.T) {
			q := gqlparser.MustLoadQueryWithRules(
				schema,
				`query foo($var: Int) { optionalIntArg(i: $var) }`,
				nil,
			)
			_, gerr := validator.VariableValues(schema, q.Operations.ForName(""), nil)
			require.NoError(t, gerr)
		...
		t.Run("Json Number -> Float", func(t *testing.T) {
			q := gqlparser.MustLoadQueryWithRules(
				schema,
				`query foo($var: Float!) { floatArg(i: $var) }`,
				nil,
			)
			vars, gerr := validator.VariableValues(
				schema,
		...
		t.Run("Bool -> Int", func(t *testing.T) {
			q := gqlparser.MustLoadQueryWithRules(
				schema,
				`query foo($var: Int!) { intArg(i: $var) }`,
				nil,
			)
			_, gerr := validator.VariableValues(
				schema,
